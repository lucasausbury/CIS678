...
cursor.execute("DROP TABLE IF EXISTS Tests_unstemmed")
cursor.execute("CREATE TABLE Tests_unstemmed ( \
...
for line in iter(f):
	...
	for i, w in enumerate(arr):
		if( i == 0 ):
			...
			sql = "INSERT INTO Tests_unstemmed(document, word, class) VALUES (%d, '%s', '%s') ON DUPLICATE KEY UPDATE count=count+1" % (document, w, actual)
			cursor.execute(sql)
			db.commit()
f.close()
db.close()